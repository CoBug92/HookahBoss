...
Create `/opt/hookahboss/.env` on the VDS with permissions `0600`. It must contain strong, device-test-only values for `HOOKAHBOSS_POSTGRES_PASSWORD` and `HOOKAHBOSS_SESSION_TOKEN_SECRET`. Do not commit or print this file. `compose.device-test.yaml` keeps Apple provider exchange disabled; authenticated Sign in with Apple is therefore unavailable in this device-test environment until real Apple provider credentials and an HTTPS callback-compatible configuration are supplied.
