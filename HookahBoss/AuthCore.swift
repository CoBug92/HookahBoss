...
enum ProtectedAction: String, CaseIterable { case favorite, rating, inventory, create, bookmark,personal; var titleKey: String { "auth.\(rawValue).title" }; var bodyKey: String { "auth.\(rawValue).body" } }
...
struct AuthGateSheet: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content.sheet(isPresented: Binding(get: { gate.state != .hidden }, set: { if !$0 { gate.cancel() } })) {
            VStack(spacing: 18) {
                if let action { Text(LocalizedStringKey(action.titleKey)).font(.title2.bold()); Text(LocalizedStringKey(action.bodyKey)).foregroundStyle(.secondary).multilineTextAlignment(.center) }
                if case .loading = gate.state { ProgressView().accessibilityLabel(Text("auth.loading")) }
                else {
                    SignInWithAppleButton(.continue) { _ in gate.begin() } onCompletion: { result in Task { await complete(result) } }
                        .signInWithAppleButtonStyle(.black).frame(height: 50).accessibilityLabel(Text("auth.apple"))
                    if case .failed = gate.state { Text("auth.error").foregroundStyle(.red); Text("auth.retryHint").font(.caption).foregroundStyle(.secondary) }
                    Button("auth.cancel") { gate.cancel() }
                }
...
final class AuthRuntime: ObservableObject {
    ...
    func setFavorite(_ enabled:Bool,mixId:UUID) async {
        ...
        catch { if SyncFailurePolicy.disposition(for:error) == .queue { enqueue(.init(id:UUID(),kind:.favorite,mixId:mixId,value:enabled ? 1:0)) } else { favoriteMixIDs=old };libraryError=String(localized:"content.error.network");saveLibraryCache() }
    }
    func setRating(_ score:Int?,mixId:UUID) async {
        ...
        catch { if SyncFailurePolicy.disposition(for:error) == .queue { enqueue(.init(id:UUID(),kind:.rating,mixId:mixId,value:score)) } else { ratings=old };libraryError=String(localized:"content.error.network");saveLibraryCache() }
    }
    func setArticleBookmark(_ enabled:Bool,slug:String)async {let old=bookmarkedArticleSlugs;if enabled{bookmarkedArticleSlugs.insert(slug)}else{bookmarkedArticleSlugs.remove(slug)};saveLibraryCache();do{try await client?.setArticleBookmark(slug:slug,enabled:enabled)}catch{if error.isRetryableSyncFailure{enqueueBookmark(.init(slug:slug,enabled:enabled))}else{bookmarkedArticleSlugs=old};libraryError=String(localized:"content.error.network");saveLibraryCache()}}
