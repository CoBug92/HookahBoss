...
final class PublicContentStoreTests: XCTestCase {
    func testArtworkMappingUsesFlavorProfilesWithDessertPriority() {
        ...
        XCTAssertEqual(MixPalette.dessert.artworkFilename,"mix-dessert")
        XCTAssertEqual(MixPalette.berry.artworkFilename,"mix-berry-citrus")
        XCTAssertEqual(MixPalette.tropical.artworkFilename,"mix-tropical")
    }

    func testArtworkPNGsAreCopiedIntoApplicationBundle() {
        for name in ["mix-dessert","mix-berry-citrus","mix-tropical"] { XCTAssertNotNil(ArtworkResource.url(named:name)) }
    }
    func testEveryArticleCategoryHasMappedBundledArtwork() {
        XCTAssertEqual(ArticleCategory.allCases.map(\.artworkFilename),["article-basics","article-preparation","article-heat","article-care","article-safety"])
        for category in ArticleCategory.allCases { XCTAssertNotNil(ArtworkResource.url(named:category.artworkFilename),category.rawValue) }
    }
