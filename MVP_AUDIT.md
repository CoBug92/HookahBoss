...
| Контекстная auth sheet, cancel без ухода, resume один раз | `AuthGate`, `AuthGateSheet`; signed-out Collection placeholders resume through `AppNavigation.openCollection` into Favorites, Personal mixes or Inventory; `AuthGateTests` plus `AppNavigationTests.testSuccessfulAuthGateResumesRequestedCollectionDestinationOnce` | proven state machine and destination integration; real Apple presentation remains release smoke |
...
| Home: mix of day, два действия, рекомендации | `HomeView`; `AppNavigation`; `MixRankingTests`; hero использует тот же `MixArtwork(palette:)` с сильным readability gradient; every recommendation is a `NavigationLink` and Home resynchronizes favorite/rating projections on return | proven logic/artwork path and recommendation→detail XCUITest |
...
| «Моё»: counters, previews, full favorites/personal/inventory, settings | `CollectionView`, `CollectionSection` destinations; favorites use typed RU/EN `L10n.Collection.favorites` | proven code; public/signed-out navigation smoke passes |
