...
| UI smoke matrix | DEBUG-only deterministic fixtures and age reset/bypass; XCUITest covers age gate plus Home, Mixes→detail/composition, filters→results, Articles→reader, signed-out Create auth prompt and My placeholders in EN light, RU dark and EN accessibility XXXL; a separate fixture renders a persisted personal-mix detail without faking Apple authentication | proven by the combined 5/5 XCUITest suite inside the 101/101 run at `/private/tmp/hb-personal-final/Logs/Test`; no authenticated Apple/Admin flow is faked |
