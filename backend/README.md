...
The latest full provenance run is `reports/provenance-sources-live-2026-09-13-mixes-v4.json`: 78/78 URLs valid, with 48 MUSTHAVE recipe pages semantically verified and 30 sources explicitly classified as reachability-only; the verifier exited zero and did not mutate seed verification dates.
...
`seeds/mixes-v3.ts` adds 14 publicly attributed user recipes hosted on the official MUSTHAVE platform. Every component is printed with an explicit numeric percentage; ratios and inferred values remain forbidden. `seeds/catalog-mix-support-v3.ts` separately records official product provenance for the six newly required catalog positions. This version brought the combined collection to 46 recipes.

`seeds/mixes-v4.ts` adds 14 more attributed MUSTHAVE recipes, covering fresh fruit, berry-herbal, tea, dessert and beverage profiles. Source pages explicitly list every percentage, and all 35 referenced products already belong to the verified catalog. The combined validator/importer processes 60 recipes transactionally and idempotently. The live provenance report semantically matched all 48 MUSTHAVE recipe URLs against their seeded component order and percentages. Run `npm run seed:validate`, `npm run seed:catalog`, `npm run seed:mixes:validate`, and `npm run seed:mixes`; running both import commands again must preserve counts.
