...
export async function archiveMissingMusthaveProducts(client:Pick<PoolClient,"query">,present:Array<{lineSlug:string;productSlug:string}>):Promise<number>{const result=await client.query(`UPDATE tobacco_products p SET status='archived',updated_at=now() FROM tobacco_lines l,brands b,content_sources s WHERE p.line_id=l.id AND l.brand_id=b.id AND p.source_id=s.id AND b.slug='musthave' AND (s.url=$1 OR s.url LIKE 'https://musthave.ru/tabak-dlya-kalyana-musthave-%') AND p.status<>'archived' AND NOT EXISTS (SELECT 1 FROM unnest($2::text[],$3::text[]) current(line_slug,product_slug) WHERE current.line_slug=l.slug AND current.product_slug=p.slug)`,["https://musthave.ru/category/tabak-dlya-kalyana/",present.map(x=>x.lineSlug),present.map(x=>x.productSlug)]);return result.rowCount??0}
...
export async function importCatalogSeed(databaseUrl: string): Promise<void> {
  ...
  try {
    ...
    const musthave=brands.find(brand=>brand.slug==="musthave");
    if(musthave)await archiveMissingMusthaveProducts(client,musthave.lines.flatMap(line=>line.products.map(product=>({lineSlug:line.slug,productSlug:product.slug}))));
