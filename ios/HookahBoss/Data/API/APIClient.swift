...
struct APIClient: Sendable {
    ...
    func products(locale: Locale = .ru, status: ProductStatus = .published, brandId: UUID? = nil) async throws -> [TobaccoProductDTO] {
        try await allCursorPages(
            "v1/products",
            query: ["locale": locale.rawValue, "status": status.rawValue, "brandId": brandId?.uuidString]
        )
    }
    ...
    func articles(locale: Locale = .ru, category: String? = nil, page: Int = 1, pageSize: Int = 20) async throws -> [ArticleDTO] {
        try await get(
            "v1/articles",
            query: ["locale": locale.rawValue, "category": category, "page": String(page), "pageSize": String(pageSize)]
        )
    }
    ...
    func exchangeAppleIdentityToken(_ identityToken: String, authorizationCode: String? = nil) async throws -> APISessionDTO {
        try await send(
            ...
            method: "POST",
            token: nil,
            body: AppleExchangeInput(identityToken: identityToken, authorizationCode: authorizationCode)
        )
    }
    func refreshSession(_ refreshToken: String) async throws -> APISessionDTO {
        try await send("v1/auth/refresh", method: "POST", token: nil, body: RefreshInput(refreshToken: refreshToken))
    }
    ...
    func deleteAccount() async throws -> AccountDeletionDTO {
        try await send("v1/me/account", method: "DELETE", token: try await requiredToken(), body: Optional<String>.none)
    }
    func adminCapabilities() async throws -> AdminCapabilitiesDTO {
        try await get("v1/me/admin-capabilities", token: try await requiredToken())
    }
    func adminList(_ resource: AdminResource) async throws -> [AdminRecordDTO] {
        let limit = 100, token = try await requiredToken(); var offset = 0, result: [AdminRecordDTO] = [], seen = Set<String>()
        while true {
            let page: [AdminRecordDTO] = try await get(
                "v1/admin/\(resource.path)",
    ...
    func adminDetail(_ resource: AdminResource, id: String) async throws -> AdminRecordDTO {
        try await get("v1/admin/\(resource.path)/\(id)", token: try await requiredToken())
    }
    func adminCreate(_ resource: AdminResource, body: [String: JSONValue]) async throws -> AdminRecordDTO {
        try await sendAuthorized("v1/admin/\(resource.path)", method: "POST", body: body)
    }
    func adminUpdate(_ resource: AdminResource, id: String, body: [String: JSONValue]) async throws -> AdminRecordDTO {
        try await sendAuthorized(
            "v1/admin/\(resource.path)\(resource.path == "substitution-deny-rules" ? "" : "/\(id)")",
            method: "PUT",
            body: body
        )
    }
    func adminDelete(_ resource: AdminResource, id: String, body: [String: JSONValue]? = nil) async throws {
    ...
    func setRating(mixId: UUID, score: Int) async throws -> RatingDTO {
        try await sendAuthorized("v1/me/ratings/\(mixId.uuidString)", method: "PUT", body: ["score": score])
    }
    func deleteRating(mixId: UUID) async throws { try await sendEmptyAuthorized("v1/me/ratings/\(mixId.uuidString)", method: "DELETE") }
    func addFavorite(mixId: UUID) async throws -> FavoriteDTO {
        try await sendAuthorized("v1/me/favorites/\(mixId.uuidString)", method: "PUT", body: Optional<String>.none)
    }
    ...
    func createPrivateProduct(_ input: PrivateProductWrite) async throws -> PrivateProductDTO {
        try await sendAuthorized("v1/me/private-products", method: "POST", body: input)
    }
    func deletePrivateProduct(id: UUID) async throws {
        try await sendEmptyAuthorized("v1/me/private-products/\(id.uuidString)", method: "DELETE")
    }
    func setArticleBookmark(slug: String, enabled: Bool) async throws {
        try await sendEmptyAuthorized("v1/me/article-bookmarks/\(slug)", method: enabled ? "PUT" : "DELETE")
    }

    func inventory() async throws -> [InventoryItemDTO] { try await get("v1/me/inventory", token: try await requiredToken()) }
    func inventoryMatches(locale: Locale = .ru) async throws -> [InventoryMatchDTO] {
        try await get("v1/me/inventory/matches", query: ["locale": locale.rawValue], token: try await requiredToken())
    }
    func upsertInventory(_ input: InventoryUpsert) async throws -> InventoryItemDTO {
        try await sendAuthorized("v1/me/inventory", method: "PUT", body: input)
    }
    ...
    func personalMix(id: UUID) async throws -> PersonalMixDetailDTO {
        try await get("v1/me/personal-mixes/\(id.uuidString)", token: try await requiredToken())
    }
    func createPersonalMix(_ input: PersonalMixWrite) async throws -> PersonalMixDetailDTO {
        try await sendAuthorized("v1/me/personal-mixes", method: "POST", body: input)
    }
    func updatePersonalMix(id: UUID, input: PersonalMixWrite) async throws -> PersonalMixDetailDTO {
        try await sendAuthorized("v1/me/personal-mixes/\(id.uuidString)", method: "PUT", body: input)
    }
    func deletePersonalMix(id: UUID) async throws {
        try await sendEmptyAuthorized("v1/me/personal-mixes/\(id.uuidString)", method: "DELETE")
    }
    ...
    private func allCursorPages<Value: Decodable & Identifiable>(_ path: String, query: [String: String?]) async throws -> [Value]
    where Value.ID == UUID {
        ...
        for _ in 0..<1000 {
            ...
            let page: CursorPageEnvelope<[Value]>
            do { page = try decoder.decode(CursorPageEnvelope<[Value]>.self, from: data) } catch {
            ...
            guard let next = page.pagination.nextCursor, !next.isEmpty, seenCursors.insert(next).inserted else {
                throw APIError.decoding("Invalid or repeated pagination cursor")
            }
            cursor = next
        }
    ...
    private func sendEmptyAuthorized(_ path: String, method: String) async throws {
        try await sendEmpty(path, method: method, token: try await requiredToken())
    }
    private func requiredToken(afterRejected token: String? = nil) async throws -> String {
        guard let tokenProvider else { throw AuthTokenError.signedOut }
        return try await tokenProvider.accessToken(afterRejectedToken: token)
    }
    ...
    private func perform<Value: Decodable>(
        ...
        do { return try decoder.decode(APIEnvelope<Value>.self, from: data).data } catch {
            throw APIError.decoding(String(describing: error))
        }
    }
    ...
    private func performData(
        ...
        do { (data, response) = try await transport.data(for: request) } catch let error as APIError { throw error } catch {
    ...
    private func performDataOnce(
        ...
        guard (200..<300).contains(response.statusCode) else {
            throw APIError.http(
                ...
                serverCode: (try? decoder.decode(ServerError.self, from: data))?.error,
                body: data
            )
        }
        return data
...
// MARK: - InventoryRemoteServing

extension APIClient: InventoryRemoteServing {}

// MARK: - AdminServing

extension APIClient: AdminServing {}
