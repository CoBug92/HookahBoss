...
final class PublicContentStore: ObservableObject {
    ...
    func load(locale: AppLocale, force: Bool = false) async {
        ...
        do {
            ...
            if mixes.isEmpty && articles.isEmpty { state = .failed(L10n.Content.Error.network) }
        }
    }
...
private struct UITestPublicTransport:APITransport {
    func data(for request:URLRequest) async throws ->(Data,HTTPURLResponse) {
        ...
        else if path.hasSuffix("/articles/care") { json=#"{"data":{"id":"50000000-0000-0000-0000-000000000002","slug":"care","title":"Hookah Care","summary":"Keep it clean","sections":[{"heading":"Cleaning routine","body":"Rinse and dry every part."}],"category":"care","readingMinutes":4,"related":[{"id":"50000000-0000-0000-0000-000000000001","slug":"guide","title":"Hookah Basics","summary":"A practical introduction","category":"fundamentals","readingMinutes":3}]}}"# }
        else if path.contains("/articles/") { json=#"{"data":{"id":"50000000-0000-0000-0000-000000000001","slug":"guide","title":"Hookah Basics","summary":"A practical introduction","sections":[{"heading":"Start","body":"Prepare carefully."}],"category":"fundamentals","readingMinutes":3,"related":[{"id":"50000000-0000-0000-0000-000000000002","slug":"care","title":"Hookah Care","summary":"Keep it clean","category":"care","readingMinutes":4}]}}"# }
        else { json=#"{"data":[{"id":"50000000-0000-0000-0000-000000000001","slug":"guide","title":"Hookah Basics","summary":"A practical introduction","category":"fundamentals","readingMinutes":3},{"id":"50000000-0000-0000-0000-000000000002","slug":"care","title":"Hookah Care","summary":"Keep it clean","category":"care","readingMinutes":4}],"pagination":{"page":1,"pageSize":100,"total":2}}"# }
        return(Data(json.utf8),HTTPURLResponse(url:request.url!,statusCode:200,httpVersion:nil,headerFields:nil)!)
    }
}
#endif

extension PublicContentStore: MixContentServing {}
extension PublicContentStore: PublicCatalogServing {
    func catalog(locale: AppLocale, force: Bool) async -> PublicCatalogSnapshot {
        await load(locale: locale, force: force)
        return PublicCatalogSnapshot(mixes: mixes, articles: articles)
    }
}
