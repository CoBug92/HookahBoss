...
enum MixRanker {
    ...
    static func mixOfDay(from mixes: [MixPreview], date: Date = Date(), calendar: Calendar = .current) -> MixPreview? {
        guard !mixes.isEmpty else { return nil }; let start = calendar.startOfDay(for: date)
        let day = calendar.dateComponents([.day], from: Date(timeIntervalSince1970: 0), to: start).day ?? 0
    ...
    private static func collectiveSignal(_ mix: MixPreview) -> Double {
        guard let rating = mix.rating else { return 0 }; let confidence = min(log10(Double(max(mix.ratingsCount, 1)) + 1) / 2, 1)
