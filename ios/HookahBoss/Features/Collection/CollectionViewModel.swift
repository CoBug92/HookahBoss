...
final class CollectionViewModel: ObservableObject {
    ...
    var favoriteMixes: [MixPreview] {
        catalogMixes.filter { favoriteMixIDs.contains($0.id) }
    }
    ...
    func requestAccess(_ action: ProtectedAction, resume: @escaping () -> Void) {
        service.requestAccess(action, resume: resume)
    }
    ...
    func deletePrivate(_ item: InventoryItem) {
        service.deletePrivateProduct(item)
    ...
    func createPrivate(
        ...
        Task {
            let succeeded = await service.createPrivateProduct(
                brand: brand,
                line: line,
                flavor: flavor,
                profiles: profiles
    ...
    func deleteAccount() {
        Task {
            do {
                showProviderUnavailable = try await service.deleteAccount()
                isAuthenticated = false
                state = .signedOut
            } catch {
                errorMessage = L10n.Content.Error.network
    ...
    private func initialize(locale: AppLocale) async {
        apply(await service.snapshot(locale: locale))
        task = nil
    }

    private func refresh() async {
        apply(await service.snapshot(locale: .currentApp))
    }
