...
@MainActor final class AppNavigationTests:XCTestCase {
    ...
    func testHomeMixFinderRequestsFiltersOnEveryTap() {
        let navigation = AppNavigation()
        navigation.openMixFinder(showFilters: true)
        XCTAssertEqual(navigation.tab, .mixes)
        XCTAssertEqual(navigation.mixFiltersRequest, 1)
        navigation.openMixFinder(showFilters: true)
        XCTAssertEqual(navigation.mixFiltersRequest, 2)
    }
