...
The archive lane prepares a local archive only; it never uploads or publishes. Its fixed App Store identifier is `ru.kostyuchenko.mixery`. Copy `scripts/.env.example` to the ignored `scripts/.env`; XcodeGen reads `TEAM_ID` from that file and writes it to `DEVELOPMENT_TEAM` (a process environment value remains available for CI). Supply the remaining signing and release configuration through environment variables such as `HOOKAHBOSS_RELEASE_API_BASE_URL`, `ARCHIVE_PATH` and `SKIP_CODE_SIGNING`. No credentials belong in the repository.
